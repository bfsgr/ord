...
bool importa(char* filename){
    ...
    char *p = bloco;
    char minibuffer[REGISTRO];
    bool frag = false;

    while(lidos > 0){
        if(!frag){
            char *l = strtok_r(bloco, DELIM_LINE, &p);

            while(l != NULL){
                if(*p == '\0') {
                    //fragmentação de registro entre buffers
                    strcpy(minibuffer, l); 
                    frag = true;
                } else {
                    if(!escreve(dat, l)){
                ...
                l = strtok_r(NULL, DELIM_LINE, &p);
            }
        } else {
            char *l = strtok_r(bloco, DELIM_LINE, &p);

            if(l != NULL){
                if(!escreve(dat, strcat(minibuffer, l))){
                    return false;
                }
            }
            l = strtok_r(NULL, DELIM_LINE, &p);
